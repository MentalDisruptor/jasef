...
class JsonConfigurationTest implements WithAssertions {
    ...
    void testStoreAndDelete() throws IOException {
        ...
        File testConfigFile = new File (basePath.getBasePath().toFile().getPath() + File.separator + "unitTest.json");
    ...
    static void cleanup() {
        BasePath basePath = new BasePath();
        File unitTestJson = new File(basePath.getBasePath().toFile().getPath() + File.separator + "unitTest.json");
